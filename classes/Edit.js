...
module.exports = {
      ...
      this.resizeClips(task)
        .then((clips) =>
        {
          ...
          mergedVideo = mergedVideo
            .addInput(`./data/intros/${task.game.name}.mp4`)
            .on("error", (err) =>
            {
              if (err) reject(err);
            });
          ...
          {
            mergedVideo = mergedVideo
              .addInput(`./data/clips/${clip.name}.mp4`)
              .on("error", (err) =>
              {
                if (err) reject(err);
              });
          ...
          mergedVideo = mergedVideo
            .addInput(`./data/intros/outro.mp4`)
            .on("error", (err) =>
            {
              if (err) reject(err);
            });
          ...
          mergedVideo
            ...
            .on("end", () =>
            {
              ...
              console.log()
              logger.success('For ' + task.game.name + ' channel.')
              console.log()
  ...
  resizeClips: function (task)
  {
    ...
    {
      let resizedPromises = [];
      let totalResize = 0; //total clips to resize
      let resizeCounter = 0; //amount of clips resized
      ...
      for (let clip of task.clips)
      {
        resizedPromises.push(
          new Promise((resolve) =>
          {
            ...
            fluent_ffmpeg.ffprobe(
              `./data/clips/${clip.name}.mp4`,
              (err, data) =>
              {
                if (err) reject(err);
